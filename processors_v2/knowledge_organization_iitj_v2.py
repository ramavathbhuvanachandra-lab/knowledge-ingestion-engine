...
def _match_aliases(
    ...
    context = _strong_context(unit)

    for entity_id, aliases in alias_map.items():
        for alias in aliases:
            if _norm(alias) in _norm(context):
                name_map = (
                    DEPARTMENT_NAMES
                    if alias_map is DEPARTMENT_ALIASES
                    else (
                        SCHOOL_NAMES
                        if alias_map is SCHOOL_ALIASES
                        else (
                            RESEARCH_PLATFORM_NAMES
                            if alias_map is RESEARCH_PLATFORM_ALIASES
                            else OFFICE_NAMES
                        )
                    )
                )
